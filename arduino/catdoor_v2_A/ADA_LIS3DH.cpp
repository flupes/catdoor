...
/**************************************************************************/
// I2C
ADA_LIS3DH::ADA_LIS3DH()
  : _cs(-1), _mosi(-1), _miso(-1), _sck(-1), _sensorID(-1)
{
  I2Cinterface = &Wire;
}

ADA_LIS3DH::ADA_LIS3DH(TwoWire *Wi)
  : _cs(-1), _mosi(-1), _miso(-1), _sck(-1), _sensorID(-1)
{
  I2Cinterface = Wi;
}

ADA_LIS3DH::ADA_LIS3DH(int8_t cspin)
  : _cs(cspin), _mosi(-1), _miso(-1), _sck(-1), _sensorID(-1)
{ }

ADA_LIS3DH::ADA_LIS3DH(int8_t cspin, int8_t mosipin, int8_t misopin, int8_t sckpin)
...
void ADA_LIS3DH::read(void) {
  ...
  if (_cs == -1) {
    ...
    accel[0] = I2Cinterface->read(); accel[0] |= ((uint16_t)I2Cinterface->read()) << 8;
    accel[1] = I2Cinterface->read(); accel[1] |= ((uint16_t)I2Cinterface->read()) << 8;
    accel[2] = I2Cinterface->read(); accel[2] |= ((uint16_t)I2Cinterface->read()) << 8;
  }
#ifndef __AVR_ATtiny85__
  else {
    if (_sck == -1)
    ...
    accel[0] = spixfer(); accel[1] |= ((uint16_t)spixfer()) << 8;
    accel[1] = spixfer(); accel[1] |= ((uint16_t)spixfer()) << 8;
    accel[2] = spixfer(); accel[2] |= ((uint16_t)spixfer()) << 8;
...
void ADA_LIS3DH::setClick(uint8_t c, uint8_t clickthresh, uint8_t timelimit, uint8_t timelatency, uint8_t timewindow) {
  if (!c) {
    //disable int
...
uint8_t ADA_LIS3DH::spixfer(uint8_t x) {
#ifndef __AVR_ATtiny85__
  if (_sck == -1)
